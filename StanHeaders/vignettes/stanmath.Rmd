...
```
LinkingTo: StanHeaders (>= 2.21.0), RcppParallel (>= 4.4.4)
```
in the DESCRIPTION file of another package and put something like
```
CXX_STD = CXX14
PKG_CPPFLAGS = -D_REENTRANT -DSTAN_THREADS `"${R_HOME}/bin/Rscript" -e "RcppParallel::CxxFlags()"`
...
```{r}
Sys.setenv(PKG_CPPFLAGS = paste("-D_REENTRANT -DSTAN_THREADS", RcppParallel::CxxFlags()))
tbb_pkg_libs <- system.file("lib", .Platform$r_arch, package = "RcppParallel")
Sys.setenv(PKG_LIBS = paste(paste0("-L", shQuote(tbb_pkg_libs)), "-ltbb"))
```
...
Sys.setenv(PKG_LIBS = paste(paste0("-L", shQuote(StanHeaders_pkg_libs)), "-lStanHeaders",
                            paste0("-L", shQuote(tbb_pkg_libs)), "-ltbb"))
```
...
exposeClass("sparselm_stan",
      ...
      file = tf,
      Rfile = FALSE)
Sys.setenv(PKG_CPPFLAGS = paste0("-D_REENTRANT -DSTAN_THREADS ",
                                 RcppParallel::CxxFlags(), " -I",

...
we need to define the objective function and its gradient with respect to $\mathbf{x}$, which we know is 
$2\left(\mathbf{x} - \mathbf{a}\right)$ in this case. At the  optimum (or on the way to the optimum), we might 
want to evaluate the Hessian matrix, which we know is $2\mathbf{I}$, but would need an additional function to 
evaluate it via autodifferentiation. Finally, one could reconceptualize the problem as solving a homogeneous
system of equations where the gradient is set equal to a vector of zeros. The `stan::math::algebra_solver` 
function can solve such a system using autodifferentiation to obtain the Jacobian, which we know to be
...
#include <stan/math/mix/mat/functor/hessian.hpp> // stuff from mix/ must come first for Hessians
#include <stan/math/fwd/mat/fun/dot_self.hpp>    // then stuff from fwd/ to evaluate dot_self()
...
y0 <- rexp(2)
stopifnot(isTRUE(all.equal(nonstiff(A, y0), c(0, 0), tol = 1e-6)))
stopifnot(isTRUE(all.equal(   stiff(A, y0), c(0, 0), tol = 1e-8)))
```
...
```
Thus, $2^{26} - 3 = 67,108,859$ is a prime number.

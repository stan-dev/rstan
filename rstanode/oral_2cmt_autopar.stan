...
functions {
  ...
  //ode-pre-include:
  //ode-post-include:
  #include "oral_2cmt_autopar_ode.stan"
  ...
  real[,] integrate_serial(real[,] y0, real[] t0,
    ...
    for(j in 1:J) {
      if(stiff) {
        res[Si_ts[j] : Si_ts[j+1] - 1] = integrate_ode_bdf(oral_2cmt_autopar_ode, y0[j], t0[j], ts[Si_ts[j] : Si_ts[j+1] - 1], theta[j], x_r[Si_x_r[j] : Si_x_r[j+1] - 1], x_i[Si_x_i[j] : Si_x_i[j+1] - 1],
                                                           rel_tol, abs_tol, max_steps);
      } else {
        res[Si_ts[j] : Si_ts[j+1] - 1] = integrate_ode_rk45(oral_2cmt_autopar_ode, y0[j], t0[j], ts[Si_ts[j] : Si_ts[j+1] - 1], theta[j], x_r[Si_x_r[j] : Si_x_r[j+1] - 1], x_i[Si_x_i[j] : Si_x_i[j+1] - 1],
                                                            rel_tol, abs_tol, max_steps);
      }
    }
    return(res);

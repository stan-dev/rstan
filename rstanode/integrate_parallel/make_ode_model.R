...
stan_ode_model <- function(stan_ode_file, cpp_ode_dir, compile=TRUE, extra_magic="") {
    ...
    stan_run_file <- sub(".stan", "_run.stan", stan_ode_file)
    cat("Creating", stan_run_file, "...\n")
    cat(code$model_code, file=stan_run_file)
    cat("\n", file=stan_run_file, append=TRUE)

    ode_name <- ode_code$name
    ## set model namespace in line with cmdstan defaults
    ##model_namespace <- sub(".stan", "_model_namespace", stan_run_file)
    model_namespace <- paste0("model_", sub(".stan", "_namespace", stan_run_file))
    model_name <- sub(".stan", "", stan_run_file)

    cpp_extra_include <- paste0(sub(".stan", "", stan_run_file), "_integrate_parallel.hpp")
...
")
    ...
    cpp_model_extra <- c(cpp_defs,
                         extra_magic,
                         readLines(file.path(cpp_ode_dir, "ode_jacobian_autogen.hpp")),
                         readLines(file.path(cpp_ode_dir, "prim_odeint_ode_system.hpp")),
                         readLines(file.path(cpp_ode_dir, "rev_odeint_ode_system.hpp")),
                         ##readLines(file.path(cpp_ode_dir, "rev_cvodes_ode_data2.hpp")),
                         ##readLines(file.path(cpp_ode_dir, "integrate_ode_rk45_bare.hpp")),
                         readLines(file.path(cpp_ode_dir, "stream_ode_observer.hpp")),
                         readLines(file.path(cpp_ode_dir, "integrate_ode_rk45_stream.hpp")),
                         readLines(file.path(cpp_ode_dir, "integrate_ode_cvodes_stream.hpp")),
                         readLines(file.path(cpp_ode_dir, "decouple_ode_states_blocked.hpp")),
                         readLines(file.path(cpp_ode_dir, "integrate_parallel.hpp"))
                         )
...
")
    ...
    if(compile) {
        ...
        sm <- stan_model(stan_run_file, model_name=model_name,

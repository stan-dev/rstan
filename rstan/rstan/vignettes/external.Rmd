...
and its first derivative. Then, we return the result of `make_callback_var()`,
whose arguments are the function value (`f`), and a functor to update the
derivative of `x` during the reverse-pass of the autodifferentiation. Here we 
calculate the first derivative of `f` (`dfdx_`) during the forward pass, and
'capture' it in the functor with `x` (`[x, dfdx_]`) for use during the reverse
pass.

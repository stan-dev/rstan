...
```
In general, the argument to the `sinc` function could either be an integer, 
a known real number, or an unknown but real parameter. Thus, it is useful to 
utilize C++ templates that can generate all versions of the `sinc` function 
with only a few lines of C++ source code. The first lines of sinc.hpp state
that the first argument is of typename `TO__` and so is the return type. The 
function signature also takes a pointer to a `std::ostream` (which is again
not used in the body of the function). The body of the `sinc` function is
simply its mathematical definition in the form of a ternary operator.
...
```
to see what function signature needed to be written for sinc.hpp.
...
and its first derivative. Then, we return the result of `precomp_v_vari`,
...
(`dfdx_`). There are other convenience functions for handling gradients that
are described 
[here](https://github.com/stan-dev/math/wiki/Adding-a-new-function-with-known-gradients).
...
RTools does not support all C++14 features. But you can use all C++11 features,

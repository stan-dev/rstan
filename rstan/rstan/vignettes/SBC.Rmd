...
generated quantities { // these adhere to the conventions above
  ...
  vector[N] log_lik;
  pars_[1] = pi_;
  for (n in 1:sum(y)) log_lik[n] = bernoulli_lpmf(1 | pi);
  for (n in (sum(y) + 1):N) log_lik[n] = bernoulli_lpmf(0 | pi);
}
```
...
log_lik <- t(sapply(1:M, FUN = function(m) {
  sum_y <- sum(y_[, m])
  c(dbinom(rep(1, sum_y), size = 1, prob = pars_[m], log = TRUE),
    dbinom(rep(1, N - sum_y), size = 1, prob = pars_[m], log = TRUE))
}))

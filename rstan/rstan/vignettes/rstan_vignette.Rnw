...
or a networked cluster. There is a \code{cores} argument to \code{stan} and \code{sampling}
that can be set to the number of chains (if the hardware has sufficient processors and RAM),
which is appropriate on most laptops.
...
via function \code{stan\_demo} in this package.

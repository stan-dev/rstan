...
\package{parallel} is different for Unix-like operating systems (for example,
Linux and Mac OS X) from Microsoft Windows. First, we provide a demonstration
...
<<parallelwin, echo=TRUE, eval=FALSE, results=hide, cache=TRUE>>=
library(rstan)
library(parallel)
f1 <- stan(file="schools.stan",data=schools_data, 
           chains=1, iter=1)
...
% Though the code for Windows is a little bit more complicated than
% that for Unix, it can also be used for Unix. So the latter code is more
% portable. 

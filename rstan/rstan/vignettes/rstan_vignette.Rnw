...
density of a posterior distribution up to an unknown additive constant.
...
on one computer using package \pkg{parallel}, provided within \R base. 


Because of the differences in different operating systems (OS), using 
\pkg{parallel} is different for Unix-like operating systems (for example,
Linux and Mac OS X) from Microsoft Windows. First, we provide some demonstration
code, which uses function \code{mclapply}, for Unix-like OS. 
...
For Windows, a different function called \code{parLapply} in
\pkg{parallel} can be used since \code{mclapply} on Windows
can use only one core.

<<parallelwin, echo=TRUE, results=hide, cache=TRUE>>=
library(rstan)
library(parallel)
f1 <- stan(file="schools.stan",data=schools_data, 
           chains=1, iter=1)
...
In the above code, we specify the same seed for all the chains but use
...
Though the above code for Windows is a bit more complicated than
the one for Unix, it can also be used for Unix. So the code using 
\code{parLapply} is more portable. 

...
* The [other vignette](http://mc-stan.org/rstan/articles/stanfit_objects.html)
for the __rstan__ package, which shows how to access the contents of stanfit
objects.
* The very thorough [Stan manual](http://mc-stan.org/users/documentation/) [@StanManual].
* The `stan_demo` function, which can be used to fit many of the example models
in the manual.
* The [__bayesplot__](http://mc-stan.org/bayesplot/) package for visual MCMC diagnostics, posterior predictive checking, and other 
plotting (ggplot based).
* The [__shinystan__](http://mc-stan.org/shinystan/) R
package, which provides a GUI for exploring MCMC output.
* The [__loo__](http://mc-stan.org/loo/) R
package, which is very useful for model comparison using stanfit objects.
* The [__rstanarm__](http://mc-stan.org/rstanarm/) R

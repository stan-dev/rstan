...
model estimation. As a \R package, \RStan\footnote{The name of 
this package is \pkg{rstan}, so we should use
\code{library(rstan)} to load it in \R.} % 
allows to use \Stan conveniently in \R (\citealt{rprj}). In general, the website
for \Stan and \RStan, \url{http://mc-stan.org}, provides the
up-to-date information about how to get started with \Stan and \RStan. For
...
\pkg{rstan}, this vignette provides pointers to many functions in package 
\pkg{rstan}, an introduction from the perspective of users. 
...
estimation in both one step and multiple steps. 
...
a model using \Stan modeling language, which is detailed 
...
Package \pkg{rstan} depends on other two \R packages: \cpkg{Rcpp} (\citealt{rcpp}) and
...
compiling the \Cpp code for \Stan model and make it possible to run the compiled model in \R.
...
First, we review the typical workflow of using \Stan as using \RStan
is similar. \Stan has a
...
\item Prepare Data  
\item Run the executable to sample from the posterior distribution
...
However, there are some differences for \pkg{rstan} compared with \Stan.
First, to simplify the interface, function \code{stan}\rstanfunidx{stan} in
package \pkg{rstan} wraps some of the above steps.
Second, in \pkg{rstan}, we compile the \Cpp code generated for a model into a 
dynamic shared object (DSO), which is also called dynamic link library (DLL),
to be loaded by \R rather than an executable program.  We will give more
details later in section~\ref{subsecmodelcompiling}.
...
steps mentioned above in \Stan. But the deep integration of \Stan in \pkg{rstan} 
...
generated from a model is almost taken away from users. Later, we refer to
these three steps by `translating', `comppiling and loading', and `sampling.' 
\begin{enumerate}[a.]\addtolength{\itemsep}{-0.6\baselineskip}
...
It is simple to just use function \code{stan} to execute the three steps of
translating, compiling and loading, and sampling. 
As these steps are implemented separately in \pkg{rstan} (detailed
in the next section), we can execute the three steps one by one.  
...
In this section, we dive into functions in \pkg{rstan} that implement
...
be called insdie \code{stan\_model} to translate the model from \Stan code to
\Cpp code.
...
\section[More details and other functions of rstan]{More details and other functions of \pkg{rstan}}

...
The package \pkg{rstan} cannot currently be installed using CRAN. Instead,
...
behaved for Hamiltonian Monte Carlo (Neal, 2011).
...
ways for model evaluation and comparison (see, e.g., Vehtari and Ojanen, 2012).
...
library(rstan)
library(parallel)
f1 <- stan(file="schools.stan",data=schols_data, 
           chains =1, iter=1)

...
Package \pkg{rstan} depends on other two \R packages: \cpkg{Rcpp} (\citealt{rcpp}),
...
But \R does not have scalars.  The behavior of \pkg{rstan} is to treat vector
...
along with other methods defined for class
\code{stanfit}. 
...
In this vignetter, we talk about the main functionality of \pkg{rstan} 
...
In general, \pkg{rstan} would always implement the interfaces to all the 

...
In this vignette, we introduce how to use \RStan, a \R package%
\footnote{The name of this package is \pkg{rstan}, so we should use
\code{library(rstan)} to load it in \R.} % 
serving as the R interface 
for \Stan---a package for obtaining Bayesian inference mainly using the
No-U-Turn sampler (NUTS) (\citealt{hoffman-gelman:2012}),
a variant of Hamiltonian Monte Carlo (HMC) (\cite{Neal:2011}). 
\Stan is open source and its information about \Stan can be found at http://mc-stan.org. 
From ``\RStan Getting Started'' (\citealt{rstangettingstarted2012}), we should be
able to successfully install \pkg{rstan} and get started to run \Stan in \R. This vignette
provides more details and introduction to many functionalities in \pkg{rstan}.


To use \Stan, we need to know Bayesian statistics (for
example, see \cite{GelmanCarlinSternRubin:2003}). Second, we 
need to know how to specify a model using \Stan modeling language (\citealt{StanManual:2013}).
As a complement to the online documentation in \pkg{rstan}, this vignette
try to connect the dots of functionalities provided by \pkg{rstan}. 

In this vignette, we start with the required packages and tools of \pkg{rstan} 
(section \ref{subsec0pre}). In section \ref{subsec0workflow}, we summarize a typical
workflow of using \RStan. In section \ref{sec0example} we use an example
to illustrate the process of using \pkg{rstan} to fit a model. 
...
Section \ref{sec0parallel} talks about how to run multiple chains in \pkg{rstan}. 
In section \ref{sec0workwstan}, we talks about some functions that \pkg{rstan}
...
from a command line, which seems hard for some users. \pkg{rstan} solves this problem
for \R users.  Although we refer to \RStan as an \R interface to \Stan, package \pkg{rstan} includes
the key part of \Stan so installing \Stan is not needed. In addition,
\pkg{rstan} can help use \Stan from command line. 

As explained later, the same as \Stan, a \Cpp compiler is needed for \RStan
to compile models.  To install a \Cpp compiler and make sure that is accessible
...
compiling the \Cpp code for \Stan model and loading the binary for later execution.
...
is quite similar in using \pkg{rstan}. \Stan defines its own
modeling language, which is similar to BUGS (\citealt{WinBUGS}) but is different
from many respects. In \Stan, we have \stanc\footnote{Using \Stan from the command line, this is an %
executable program.} to translate the model 
in \Stan modeling language to \Cpp code, which can be compiled to an executable 
program. Then we run this program to draw samples.  If the model has data,  
a special format can be given for this program. In summary, the following are
typical steps of using \Stan. 
...
In \pkg{rstan}, we have almost the same steps as in \Stan but packaged 
in \R functions though some steps can be done in one function of
\pkg{rstan}.  Knowing these steps is helpful sometimes 
for example when we encounter problem and want to identify the problematic step. 
...
package \pkg{rstan} wraps up some of the above steps.
...
rather than an executable binary file.  We will talk about more details later.


\section{An example of using \pkg{rstan}}
\label{sec0example} 

In this section, we use an example to demonstrate using \pkg{rstan}, the same
example as in \cite{rstangettingstarted2012}. 
...
However, for better performance of sampling, instead we would use
the following equivalent code for this model. See the manual of Stan for more
details. 
...
Often we always have the elements of the data for a model
in our workspace. \pkg{rstan} allows a  convenient way to just 
specify the names instead of creating a new list. The following
...
\subsection{Sample for the model} 
...
Function \code{stan} wraps up the following three steps, which are similar to some
...
If there is no error with all the above, the returned \code{stanfit} 
...
In the case there is an error, \code{stan} will either quit 
for example we have syntax error in our \Stan code or
...
time for compiling the model when for example we have the problems with
specifying the data or inital values. 
For a \code{stanfit} object, many methods such as \code{print} and \code{plot} 
...
It is simple to just use function \code{stan} to conduct all of the above
...
In next section, we talk about the steps that function \code{stan} is made up  
...
just using function \code{stan}. However, what \code{stan} does is just wrapping 
up a few functions that implement different steps of using \Stan.  
So \pkg{rstan} also allow us to fit a model step by step as in \Stan. 

First, we can use \code{stanc}\rstanfunidx{stanc} function to translate the model in \Stan
...
use function \code{stan\_model}\rstanfunidx{stan\_model} to compile the \Cpp code to a DSO and load 
the DSO into \R. \pkg{rstan} represents a compiled model using S4 class \code{stanmodel}. 
In this step (as in calling \code{stan}), the \Cpp compiler might spew out 
a lot of intermediate message such as the \Cpp code and warning
message especially if \code{verbose=TRUE}. 
...
that provides the \Stan code for a model, \code{stanc} will
...
which is used to sample from the posterior
distribution with corresponding data and specification of sampling
parameters such as \code{chains}. 
...
In this section, we begin with talking about some typical arguments of function \code{stan}
and data preprocessing that is needed \pkg{rstan} to allow \Stan to read 
data defined in \R. 
Also we talk about S4 class \code{stanfit} 
in more details, in particular other methods that can be used to diagnose 
the convergence of Markov chains, to example samples, and to obtain 
...
in which the NUTS sampler (or other samplers implemented in \Stan) is in the phase
of adaptation (hence the sample should not be used for inference as it may not
...
are calculated using the sample after warmup period. 

When the user is trying to specify the initial values for the parameters (i.e., 
specifying \code{init} as a list of lists), currently, \pkg{rstan} does not
allow partially specifying initial values so that initial values for all
parameters need to be given. 

Through changing some of augments for calling function \code{stan}, we
can use other samplers such as HMC implemented by \Stan. All the details
can be found in the help documentation of function \code{stan}. As \Stan
and \pkg{rstan} is being actively developed, the help documentation in package
\pkg{rstan} should always have the up-to-date details.


Also the test gradient mode in \Stan can be used through passing 
argument \code{test\_gard=TRUE}. As in \Stan, the test gradient mode
...
both the algorithmic differentiation in \Stan and finite difference.
...
\Stan has its own specification of data. \pkg{rstan} pre-processes 
the data expressed in \R for input of \Stan. Here we talk
about a few important steps in the data preprocessing (see the
help of function{stan} for a full reference).
\Stan modeling language differentiates integer and double input of data
and variables. But typically as a user, we are always using type of \code{double}
in \R since that is the default in \R. So an important data preprocessing step inside function 
\code{stan} is to convert some data to type of \code{double} if possible. 

In \Stan is we have type scalar and other types that are a set
of scalars (for example, type vector and a matrix). 
But in \R, we do not have type scalar.  The behavior in \pkg{rstan} is to treat vector
length 1 to be a scalar. However, we might have a model with data block 
defined in figure~\ref{fig0datablock}, in which $N$ can be $1$ in some cases.
So we know that $N$ is always larger than $1$, we can a vector of length $N$ in \R
as the data input for $y$. If we want to make our input of $y$ is still good when $N=1$,
...
preprocessing is to check missing values and issue an error if found. 


\subsection{Methods of class \code{stanfit}} 

For the fitted object, S4 class \code{stanfit}, 
we have defined methods such as \code{print},
\code{summary}, \code{plot}, and \code{traceplot}. 
...
the split $\hat{R}$.\footnote{Split $\hat{R}$ is a revised version of 
$\hat{R}$ statistic proposed in \cite{GelmanRubin:1992}: the split $\hat{R}$ 
is based on splitting each chain into two halves. See \Stan manual for the details.} %
The inference including mean, standard deviation, quantiles of interest, split $\hat{R}$, and 
effective sample sizes based on the samples after warmup phase can be obtained by \code{summary}\rstanfunidx{summary}
function.  Function \code{print}\rstanfunidx{print} prints some of the summaries for all chains combined and function
\code{plot}\rstanfunidx{plot} provides an overview plot, which are 
demonstrated in Section \ref{sec0example}). 
...
In this plot, an credible interval (by default 80\%) for all the parameters
as well as \code{lp\_\_}, 
...
Function \code{traceplot}\rstanfunidx{traceplot} plot the traces of all chains for the parameters 
...
different ways to access the samples. If \code{permuted=TRUE}
...
In addition, methods of \code{as.array}\rstanfunidx{as.array}, \code{as.matrix}\rstanfunidx{as.matrix}, and
\code{as.data.frame}\rstanfunidx{as.data.frame} are defined for \code{stanfit} object. These
function returns the draws of samples from the posterior
distribution in forms of a 3-D array or a matrix.  The
...
creates a matrix of output plots from a \code{stanfit} object, which
can help diagnose convergence as well. 
...
These methods are listed in table \ref{tab0stanfitfuns} along with 
other methods defined for class \code{stanfit}. 

Last, a common feature for some of functions for class \code{stanfit} is that they allow us 
...
could be helpful when there are too many parameters in the model. For instance,
...
Essentially, as we can defined the log of the probability
density of the posterior distribution up to additive constant (coded
as \code{lp\_\_} in \Stan), 
we can use \Stan to sample from the posterior distribution. 
...
function \code{get\_num\_upars}\rstanfunidx{get\_num\_upars} for a stanfit object can be used. 
...
such as stochastic MAP estimation for Bayesian models in \R.


\begin{table}
\caption{Methods for S4 class \code{stanfit}} 
...
\code{as.array}        & \multirow{3}{\linewidth}{extract the samples excluding warmup to a 3-D array, matrix, data.frame}  \\
\code{as.matrix}       & \\
\code{as.data.frame}   & \\
...
Last, there are some options that configures compiling the \Cpp code to a DSO
...
can be found in the online document.
In particular, we strongly suggest using \code{set\_cppo('fast')} 
...
\code{chains}. \pkg{rstan} runs each chain serially using one 
...
result, if we can run multiple chains in parallel using a vareity of approaches provided by 
...
Note in the above code, we specify the same for all the chains but use
...
to work with \Stan. 
First, since \Stan only supports a subset syntax of \R dump data formats, 
function \code{stan\_rdump} dumps the data in \R to the format that is 
...
in \pkg{rstan} creates
a \code{stanfit} object from reading the comma separated files (CSV) generated
if we use \Stan from the command line. As a result, we can use many methods
defined of \code{stanfit} class to work with the samples. 
...
details and includes a vareity of model examples. \pkg{rstan} provides
function \code{stan\_demo}\rstanfunidx{stan\_demo} to choose and then run an example included in \Stan. 
Some of these examples are talked in \Stan's manual and some of the model
originated from the examples of BUGS (\citealt{WinBUGS}). 
...
And for more help, check out the information at Stan's website (http://mc-stan.org). 

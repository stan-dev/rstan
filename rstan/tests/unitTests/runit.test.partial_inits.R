...
test_partial_inits <- function() {
  ...
  for (i in seq_along(i1)) {
    checkEquals(i1[[i]]$mu, 0)
    checkEquals(i1[[i]]$alpha, 0)
    checkEquals(i1[[i]]$eta, 0)
  ...
  for (i in seq_along(i3)) {
    ...
    # the following randomly
    checkTrue(!identical(i3[[i]]$alpha, 0))
    checkTrue(!identical(i3[[i]]$alpha, 1))
    checkTrue(!identical(i3[[i]]$eta, 0))
    checkTrue(!identical(i3[[i]]$eta, 1))
    checkTrue(!identical(i3[[i]]$sigma, 1))
    checkTrue(!identical(i3[[i]]$sigma, 2))
    checkTrue(!identical(i3[[i]]$delta, as.array(rep(1,3) / 3)))
    checkTrue(!identical(i3[[i]]$delta, as.array(c(.1, .2, .7))))
  }

  # allow partial inits, but mu is not specified correctly
  f4 <- sampling(rr, data = dat, iter = 10, chains = 1,
  ...
  f6 <- sampling(rr, data = dat, iter = 10, chains = 1,
                 seed = 3, thin = 1, init_r = 4,
                 enable_random_init = TRUE)

  f7 <- sampling(rr, data = dat, iter = 10, chains = 1,
  ...
  f8 <- sampling(rr, data = dat, iter = 10, chains = 1,
  ...
  f9 <- sampling(rr, data = dat, iter = 10, chains = 2, seed = 3,
                 ...
                 enable_random_init = TRUE)
  f10 <- sampling(rr, data = dat, iter = 10, chains = 2, seed = 3,

...
These functions are improved versions of the traditional Rhat (for
...
- sims:
  ...
  A two-dimensional array whose rows are equal to the number of
  iterations of the Markov Chain(s) and whose columns are equal to the
  number of Markov Chains (preferably more than one). The cells are the
  realized draws for a particular parameter or function of parameters.
...
tails of the distribution (related e.g. to efficiency of variance and
tail quantile estimates).

Both bulk-ESS and tail-ESS should be at least \\100\\ (approximately)

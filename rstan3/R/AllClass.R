...
# FIXME: Should this be an initialize method?
data = function(data, search.env = parent.env(.self), inherits.env = FALSE) {
  fields <- tail(names(.self$getRefClass()$fields()), -4L)
  if (length(data) == 0) {
    data <- mget(fields, envir = search.env, inherits = inherits.env,
                    ifnotfound = list(function(...) NULL))
  }
  nms <- names(data)
  for (i in seq_along(data)) if (!is.null(data[[i]])) {
    if (nms[i] %in% fields)
      .self$field(nms[i], as(data[[i]], class(.self$field(nms[i]))))
    else
...
build = function() { 
  nms <- names(.self$getRefClass()$fields())[-c(1:3)]
  ...
  mod <- new(.self$field(nms[1]), datlist, .self$seed, dummy)
...
#' Constructor
#' 
#' @param ... Objects passed to the data block of the Stan program
...
#' @param seed An integer for Stan's PRNG seed
#' @export
stan_build <- function(...,
                       file = file.choose(),
  ...
  allow_undefined <- length(includes) > 0
  ...
  SPWD <- setRefClass(md5, contains = "StanBuild", fields = fields, 
                      where = .GlobalEnv)
  SPWD$lock(c("shared.object", obj_name, "seed"))
  ...
  m <- match.call()
  dotlist <- list(...)
  no_names <- is.null(names(dotlist))
  has_name <- if (no_names) FALSE else nzchar(names(dotlist))
  m$file <- m$model_name <- m$model_code <- m$includes <- m$seed <- NULL
  nms <- as.character(m)[-1L]
  if (no_names) names(dotlist) <- nms
  else names(dotlist)[!has_name] <- nms[!has_name]
  out$data(dotlist)
  return(out$build())
}
